...
export default function CallItems() {
  ...
  // Fechar o modal de transcrição  const handleCloseTranscriptionModal = () => {
            ...
            <div className="flex flex-col space-y-3">
              ...
              {calls && calls.find((c: any) => String(c.avaliacao_id) === String(avaliacaoId))?.callerid && (
                <div className="flex items-center text-gray-600">
                  ...
                  <span className="text-sm">Cliente: <span className="font-medium text-gray-800">{calls.find((c: any) => String(c.avaliacao_id) === String(avaliacaoId))?.callerid}</span></span>
                </div>
              )}
            </div>
          </div>
